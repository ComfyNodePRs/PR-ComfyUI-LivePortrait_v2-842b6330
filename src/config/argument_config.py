...
class ArgumentConfig(PrintableConfig):
    ########## input arguments ##########
    source: Annotated[str, tyro.conf.arg(aliases=["-s"])] = make_abs_path('../../assets/examples/source/s12.jpg')  # path to the source portrait (human/animal) or video (human)
    driving:  Annotated[str, tyro.conf.arg(aliases=["-d"])] = "custom_nodes/ComfyUI-LivePortrait_v2/assets/examples/driving/d9.mp4"  # path to driving video or template (.pkl format)
    #autodl-tmp/ComfyUI/custom_nodes/ComfyUI-LivePortrait_v2/assets/examples/driving/d9.mp4
 ...
 ########## gradio arguments ##########
    port: Annotated[int, tyro.conf.arg(aliases=["--port"])] = 6006  # port for gradio server
    listen: bool = True
