...
class LivePortraitPipeline(object):
    ...
    def make_motion_template(self, I_lst, c_eyes_lst, c_lip_lst, **kwargs):
        ...
        template_dct = {
            ...
            'motion': [],
            'c_eyes_lst': [],#not in animal
            'c_lip_lst': [],#not in animal
        }
        ...
        for i in track(range(n_frames), description='Making motion templates...', total=n_frames):
            ...
            item_dct = {
                ...
                'kp': x_i_info['kp'].cpu().numpy().astype(np.float32),#not in animal
                'x_s': x_s.cpu().numpy().astype(np.float32),#not in animal
            }

            template_dct['motion'].append(item_dct)

            c_eyes = c_eyes_lst[i].astype(np.float32)#not in animal
            template_dct['c_eyes_lst'].append(c_eyes)#not in animal

            c_lip = c_lip_lst[i].astype(np.float32)#not in animal
            template_dct['c_lip_lst'].append(c_lip)#not in animal
...
################################################################################
        elif is_video(args.source):#not in animal
...
################################################################################
        ...
        if flag_load_from_template:
            ...
            ####################################################################
            flag_is_driving_video = True if driving_n_frames > 1 else False#    not in animal
        ...
        ######## process source info ########
        if flag_is_source_video:#not in animal
            ...
            # let lip-open scalar to be 0 at first not in animal
